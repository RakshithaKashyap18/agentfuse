...
- **Design fix applied during T13:** as originally specified, a tripped LoopBreaker/StallDetector blocked a run forever — blocked calls add no events, so the streak never cleared and the spec's "blocking can heal the agent" demo could not pass. Fix: `Window.last_block_ts` (the run's most recent BLOCK/KILL incident ts, via `Store.last_block_ts`) and the streak-based policies only count events at/after it. A block resets the streak, the agent gets a fresh chance after reading the guidance, and relentless looping re-trips the breaker. T10's end-to-end loop test was updated to assert trip → heal → re-trip.
